1. Add fields to the .agent fields, don't change the existing ones
2. on upgrade bump the coordinator index
3. on upgrade set the default api to empty -> force all API keys to be regenerated
4. on agent checkin, after upgrade, try to search api key by metadata.agentId and invalidate the "old" ones
5. ensure we delete the old API keys
6. fix painless script
7. add output name or something to api key metadata




Thu 14 Jul 16:09:41 CEST 2022

so far I could drill the problem down to how the output permissions change.
