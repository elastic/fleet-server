...
// TestEnrollCreateVersionConflict_DocumentNotFound_Retries verifies that when
// ES returns a 409 version conflict on Create (e.g. due to a primary shard
// relocation) but the document is not found on re-read, Fleet Server retries
// the Create and ultimately succeeds.
func TestEnrollCreateVersionConflict_DocumentNotFound_Retries(t *testing.T) {
	ctx := t.Context()
	rb := &rollback.Rollback{}
	zlog := zerolog.Logger{}
	...
	// No existing agent with this enrollment ID.
	bulker.On("Search", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(&es.ResultT{
		HitsT: es.HitsT{Hits: make([]es.HitT, 0)},
	}, nil)
	bulker.On("APIKeyCreate", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(
		&apikey.APIKey{ID: "key-1", Key: "key-1"}, nil)

	// First Create returns 409; second succeeds.
	bulker.On("Create", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return("", es.ErrElasticVersionConflict).Once()
	bulker.On("Create", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return("doc-id", nil).Once()

	// ReadRaw (called by dl.GetAgent after the 409): agent not found.
	bulker.On("ReadRaw", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(
		(*bulk.MgetResponseItem)(nil), es.ErrElasticNotFound)

	resp, err := et._enroll(ctx, rb, zlog, req, "policy-1", []string{}, "8.9.0")
	assert.NoError(t, err)
	assert.Equal(t, "created", resp.Action)
	bulker.AssertNumberOfCalls(t, "Create", 2)
	bulker.AssertNumberOfCalls(t, "ReadRaw", 1)
}

// TestEnrollCreateVersionConflict_DocumentFound_SameAPIKey_Succeeds verifies that
// when ES returns a 409 on Create but the document is found on re-read with the
// same access_api_key_id (i.e. our Create was committed before the conflict was
// reported), Fleet Server treats the enrollment as successful.
func TestEnrollCreateVersionConflict_DocumentFound_SameAPIKey_Succeeds(t *testing.T) {
	ctx := t.Context()
	rb := &rollback.Rollback{}
	zlog := zerolog.Logger{}
	enrollmentID := "test-enrollment-id"
	...
	// Create returns 409 (document was actually committed on the old primary).
	bulker.On("Create", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return("", es.ErrElasticVersionConflict)

	// ReadRaw (called by dl.GetAgent): document found with the same access_api_key_id.
	bulker.On("ReadRaw", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(
		&bulk.MgetResponseItem{
			Found:  true,
			Source: []byte(`{"access_api_key_id":"key-1"}`),
		}, nil)

	resp, err := et._enroll(ctx, rb, zlog, req, "policy-1", []string{}, "8.9.0")
	assert.NoError(t, err)
	assert.Equal(t, "created", resp.Action)
	// Create is called once; the 409 is resolved by re-reading the document.
	bulker.AssertNumberOfCalls(t, "Create", 1)
	bulker.AssertNumberOfCalls(t, "ReadRaw", 1)
}
